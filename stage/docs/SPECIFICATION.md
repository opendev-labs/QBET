...
### Keywords
- **Physical Keywords**: `allocate`, `deallocate`, `call`, `return`, `wait`, `time`, `causality`, `collapse`, `failure`
- **Semantic Keywords**: `manifest`, `use`, `function`, `state`, `observe`, `flow`, `bind`, `universe`, `within`, `when`, `into`, `qubit`, `karma`, `dharma`, `entangle`

### 2. Lexical Grammar
The engine processes **Semantic Keywords** and translates them into **Physical Primitives**.
...
A QBET program executes within a Reality State $\mathcal{R}$, which is a mapping of identifiers to **Potential Fields** ($\mathcal{P}$), governed by the physical laws defined in `c.qbet`.

### Layered Hierarchy
1. **Physical Laws (`c.qbet`)**: Defines memory, time, causality, and execution primitives.
2. **Language Semantics (`engine.qbet`)**: Implements metaphors and high-level logic (e.g., qubits, waves).
3. **System layer (`system.qbet`)**: Commands, portals, and OS-level interactions.
4. **User layer (`user.qbet`)**: Application logic and universe manifestation.
